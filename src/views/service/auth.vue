<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column :label="$t('services.auth.id')" prop="id" align="center" width="80">
        <template slot-scope="scope">
          <span>{{ scope.$index + 1 }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('services.auth.name')" min-width="150px">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.account }}</span>
        </template>
      </el-table-column>
      <el-table-column v-if="showPassword" :label="$t('services.auth.password')" min-width="110px" align="center">
        <template slot-scope="scope">
          <span style="color:red;">{{ scope.row.password }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('services.auth.actions')" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button size="mini" type="danger" @click="handleDelete(row)">
            {{ $t('services.auth.delete') }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
